...
(function () {
    ...
    panelStyle.textContent = `
        ...
        .sp-dd-select {
            flex: 1; background: var(--card); border: 1px solid rgba(255,255,255,0.12);
    ...
    // ── Dropdown factory ───────────────────────────────────────────────
    function makeDropdown(container, label, items, defaultIdx, onChange) {
    ...
    makeDropdown(
        document.getElementById('sp-font-dd'),
        'Font', FONTS, FONT_DEFAULT, applyFont
    );
    ...
    function applyVariant(idx) {
        ...
        if (cta) cta.dataset.variant = ['A', 'B', 'C', 'D'][idx];
    }

    makeDropdown(
        document.getElementById('sp-variant-dd'),
        'Stil butoane', VARIANTS, VARIANT_DEFAULT, applyVariant
    );
    ...
    // ── Sliders — ranges centered on defaults for fine tuning ──────────
    // BASE matches screenshot values
    const BASE = {
    ...
    const SLIDERS = [
        // Darkness/percentage sliders — tight range centered on default
        ...
        // Hue sliders — full range, no change
        ...
        // Overlay sliders — tight range around defaults
